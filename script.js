...
function showTodo(filter) {
    ...
    if (todos) {
        todos.forEach((todo, id) => {
            ...
            if (filter == todo.status || filter == "all") {
                liTag += `<li class="task" style="border-bottom: solid var(--bordergray) 1.5px !important;">
                <label style="user-select: all;" for="${id}">
                    <input onclick="updateStatus(this)" class="form-check-input" type="checkbox" id="${id} todo-input" ${completed}>
                    <p class="${completed}" style="text-transform: capitalize;">${todo.name}</p>
                </label>
                <div class="settings">
                    <button class="btn dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false"></button>
                  <ul class="task-menu dropdown-menu">
                    <li><a onclick='editTask(${id}, "${todo.name}")' id="dropdowntxt" class="dropdown-item Edit">Edit</a></li>
                    <li><a onclick='deleteTask(${id}, "${filter}")' class="dropdown-item Delete">Delete</a></li>
                  </ul>
                </div>
            </li>`;
            }
    ...
    taskBox.innerHTML = liTag || `<span style="user-select: none;"><b  style="color: #aaa !important;">No Tasks.</b></span>`;
...
clearAll.addEventListener("click", () => {
    var delAns = confirm("Clear all tasks?");
    if (delAns == true) {
        isEditTask = false;
    ...
    function createMonthTable(data, option) {
        ...
        for (r = 3; r <= 7; r = r + 1) {
            ...
            for (c = 0; c <= 6; c = c + 1) {
                ...
                if (option.date === count && option.month === new Date().getMonth() && option.year == new Date().getFullYear() && option.highlighttargetdate === true) {
                    td.setAttribute("class", "dycalendar-target-date");
                }
                count = count + 1;
                tr.appendChild(td);
...
if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
    ...
    document.getElementById("input").classList.add('dark-mode');
    document.getElementById("dropdowntxt").classList.add('dark-mode');
    document.getElementById("linkOut").classList.add('dark-mode');

    const formcheckinput = document.getElementsByClassName("form-check-input");
    for (let i = 0; i < formcheckinput.length; i++) {
        formcheckinput[i].classList.add('dark-mode');
    }
    const offcanvas = document.getElementsByClassName("offcanvas");
    for (let i = 0; i < offcanvas.length; i++) {
        offcanvas[i].classList.add('dark-mode');
    }
    const calencontainer = document.getElementsByClassName("calencontainer");
    for (let i = 0; i < calencontainer.length; i++) {
        calencontainer[i].classList.add('dark-mode');
    }
    const dycalendarspanmonthyear = document.getElementsByClassName("dycalendar-span-month-year");
    for (let i = 0; i < dycalendarspanmonthyear.length; i++) {
        dycalendarspanmonthyear[i].classList.add('dark-mode');
    }
    const filters = document.getElementsByClassName("filters");
    for (let i = 0; i < filters.length; i++) {
        filters[i].classList.add('dark-mode');
    }
}
window.matchMedia('(prefers-color-scheme: dark)').addListener((e) => {
    const darkModeOn = e.matches;
    document.body.classList.toggle('dark-mode', darkModeOn);
    document.getElementById("input").classList.toggle('dark-mode', darkModeOn);
    document.getElementById("dropdowntxt").classList.toggle('dark-mode', darkModeOn);
    document.getElementById("linkOut").classList.toggle('dark-mode', darkModeOn);

    const formcheckinput = document.getElementsByClassName("form-check-input");
    for (let i = 0; i < formcheckinput.length; i++) {
        formcheckinput[i].classList.toggle('dark-mode', darkModeOn);
    }
    const offcanvas = document.getElementsByClassName("offcanvas");
    for (let i = 0; i < offcanvas.length; i++) {
        offcanvas[i].classList.toggle('dark-mode', darkModeOn);
    }
    const calencontainer = document.getElementsByClassName("calencontainer");
    for (let i = 0; i < calencontainer.length; i++) {
        calencontainer[i].classList.toggle('dark-mode', darkModeOn);
    }
    const dycalendarspanmonthyear = document.getElementsByClassName("dycalendar-span-month-year");
    for (let i = 0; i < dycalendarspanmonthyear.length; i++) {
        dycalendarspanmonthyear[i].classList.toggle('dark-mode', darkModeOn);
    }
    const filters = document.getElementsByClassName("filters");
    for (let i = 0; i < filters.length; i++) {
        filters[i].classList.toggle('dark-mode', darkModeOn);
    }
});